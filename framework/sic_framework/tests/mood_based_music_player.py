...
def play_music(file_path):
    ...
    message = AudioMessage(
        sample_rate=samplerate * 2, waveform=sound
    )  # Adjust sample_rate if needed
    pepper.speaker.send_message(message)

    print("Audio sent, without waiting for it to complete playing.")


# handle different moods and select corresponding music
...
# START DEMO
pepper.motion.request(NaoPostureRequest("Stand", 0.5))
time.sleep(1)
...
try:
    for i in range(25):
        ...
        mood = reply.mood
        print("Intent:", intent)
        print("Mood: ", mood if mood else "no mood detected")

        if mood:  # if mood parameter in response
            response_text, music_file_path = handle_mood(mood)
            print("Response:", response_text)
            pepper.tts.request(NaoqiTextToSpeechRequest(response_text))
            play_music(music_file_path)  # play music matching the mood
            time.sleep(25)
            mood = None
